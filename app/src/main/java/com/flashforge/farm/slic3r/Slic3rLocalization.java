...
public class Slic3rLocalization {
    ...
    public Slic3rLocalization(String key) throws IOException {
        InputStream in = FarmApp.INSTANCE.getAssets().open("localization/" + key + ".po");
    ...
    private static String replaceStr(String val) {
        if (val == null) return "";
        String appName = FarmApp.INSTANCE != null
                ? FarmApp.INSTANCE.getString(R.string.AppName) : "FlashForge Farm";
        return val.replace("\\n", "\n").replaceAll("\\\\(.)", "$1")
                .replaceAll("(?i)slic3r", appName)
                .replaceAll("(?i)prusasl?i[sc]er", appName);
    }
