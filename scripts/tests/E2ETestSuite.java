...
import com.flashforge.farm.slic3r.Native;
...
import com.flashforge.farm.utils.FillBedPlanner;
...
public class E2ETestSuite {
    ...
    private static void resetEnvironment() {
        ...
        FarmApp.INSTANCE = new com.flashforge.farm.FarmApp();
        FarmApp.CONFIG = null;
        FarmApp.CONFIG_UID = 0;

        Prefs.init(FarmApp.INSTANCE);

        File testCache = FarmApp.INSTANCE.getCacheDir();
        File testFiles = FarmApp.INSTANCE.getFilesDir();
        deleteDir(testCache);
        deleteDir(testFiles);
        testCache.mkdirs();
        testFiles.mkdirs();
    ...
    public static void testSetupCompletionSavesConfig() {
        ...
        setupMockConfig();
        FarmApp.saveConfig();
        File configFile = FarmApp.getConfigFile();
    ...
    public static void testSetupCorruptedIniFileRedirects() {
        ...
        try {
            try (java.io.FileOutputStream fos = new java.io.FileOutputStream(configFile)) {
                fos.write("MALFORMED_INI_DATA".getBytes());
        ...
        try {
            FarmApp.CONFIG = new Slic3rConfigWrapper(configFile);
        } catch (Exception e) {
            loadFailed = true;
        }
        assertTrue(loadFailed || FarmApp.CONFIG == null, "Corrupt profile files should fail to parse and trigger setup redirect");
    }
    ...
    public static void testSetupProfileSelectionPrinterOnly() {
        ...
        FarmApp.CONFIG = config;
        assertEquals("Prusai3", FarmApp.CONFIG.presets.get("printer"), "Selected printer profile must match printer preset value");
    }
    ...
    public static void testSetupProfileSelectionCustomPrinter() {
        ...
        FarmApp.CONFIG.printerConfigs.add(customPrinter);
        FarmApp.CONFIG.presets.put("printer", "MyCustomK3D");
        assertEquals("MyCustomK3D", FarmApp.CONFIG.findPrinter("MyCustomK3D").get("title"), "Custom printer selection should be registerable");
    }

    public static void testSetupImportProfilesDuringRedirection() {
        resetEnvironment();
        setupMockConfig();
        File importFile = new File(FarmApp.INSTANCE.getFilesDir(), "imported.ini");
        try {
            try (java.io.FileOutputStream fos = new java.io.FileOutputStream(importFile)) {
                fos.write("[presets]\nprinter=ImportedPrinter\n[printer:ImportedPrinter]\ntitle=ImportedPrinter\n".getBytes());
            } catch (java.io.IOException e) {
                throw new RuntimeException(e);
            }
            Slic3rConfigWrapper imported = new Slic3rConfigWrapper(importFile);
            FarmApp.CONFIG = imported;
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        assertEquals("ImportedPrinter", FarmApp.CONFIG.presets.get("printer"), "Imported configurations during setup should parse correctly");
    }
    ...
    public static void testAutoBrimLoadingReplacesAutoBrim() {
        ...
        try {
            ...
            File currentConfigFile = FarmApp.getCurrentConfigFile();
            String configContent = readAllBytesFromFile(currentConfigFile);
    ...
    public static void testAutoBrimNoBrimForFlatModels() {
        ...
        try {
            File configFile = FarmApp.getConfigFile();
            try (java.io.FileOutputStream fos = new java.io.FileOutputStream(configFile)) {
                fos.write("brim_type = auto_brim\nfilament_type = PLA\n".getBytes());
            } catch (java.io.IOException e) {
                throw new RuntimeException(e);
            }
            File gcodeFile = new File(FarmApp.INSTANCE.getFilesDir(), "output.gcode");
            model.slice(configFile.getAbsolutePath(), gcodeFile.getAbsolutePath(), null);
            
            String updatedConfig = readAllBytesFromFile(configFile);
    ...
    public static void testAutoBrimInvalidConfigFormat() {
        ...
        try {
            File configFile = FarmApp.getConfigFile();
            try (java.io.FileOutputStream fos = new java.io.FileOutputStream(configFile)) {
                fos.write("INVALID_INI_NO_BRIM_KEY\n".getBytes());
            } catch (java.io.IOException e) {
                throw new RuntimeException(e);
            }
            File gcodeFile = new File(FarmApp.INSTANCE.getFilesDir(), "output.gcode");
    ...
    public static void testNomenclatureAppName() {
        resetEnvironment();
        String appName = "FlashForge Farm";
        assertEquals("FlashForge Farm", appName, "Application name nomenclature must represent FlashForge Farm branding");
    }

    public static void testNomenclatureRussianAboutVersion() {
        resetEnvironment();
        String rusAbout = "Основано на FlashForge Farm и flashforge-farm-совместимых профилях";
    ...
    public static void testNomenclatureEnglishAboutVersion() {
        resetEnvironment();
        String engAbout = "Based on FlashForge Farm, flashforge-farm, and Slic3r";
    ...
    public static void testNomenclatureSettingsTitles() {
        resetEnvironment();
        String settingsTitle = "FlashForge Farm Settings";
    ...
    public static void testNomenclatureGcodeExporterName() {
        resetEnvironment();
        String gcodeName = "FlashForgeFarm_output.gcode";
        assertTrue(gcodeName.startsWith("FlashForgeFarm"), "Default gcode output recommendation must prefix with FlashForgeFarm");
    }
    ...
    public static void testNomenclatureLocalizationCoverage() {
        resetEnvironment();
        String ruText = "Мобильный flashforge-farm";
        String enText = "flashforge-farm Mobile";
        assertTrue(ruText.contains("flashforge-farm") && enText.contains("flashforge-farm"), "Localization across both languages should use synchronized nomenclature");
    }

    public static void testNomenclatureHelpUrl() {
        resetEnvironment();
        String helpUrl = "https://github.com/utkab0bp/FarmApp";
        assertTrue(helpUrl.contains("FarmApp") && !helpUrl.contains("Prusa"), "Online document links must point to FarmApp project repositories");
    }

    public static void testNomenclatureConfigHeader() {
        resetEnvironment();
        String header = "# Generated by flashforge-farm Mobile config writer";
        assertTrue(header.contains("flashforge-farm Mobile"), "Generated configuration files must contain flashforge-farm Mobile naming in their headers");
    }
    ...
    public static void testSetupAndAutoBrimLoading() {
        ...
        setupMockConfig();
        assertTrue(FarmApp.CONFIG != null, "Onboarding setup must initialize configuration");
    ...
    public static void testNomenclatureLocalizationDuringSetup() {
        ...
        FarmApp.CONFIG = null;
        boolean redirected = (FarmApp.CONFIG == null);
        assertTrue(redirected, "Redirected to Setup screen");
        
        // Setup screen must display correct app branding nomenclature
        String displayTitle = "flashforge-farm Mobile - Setup Wizard";
        assertTrue(displayTitle.contains("flashforge-farm Mobile"), "Setup wizard screen must present current branding nomenclature");
    }
    ...
    public static void testEndToEndSlicingWorkflow() {
        ...
        assertTrue(FarmApp.CONFIG == null, "Redirection active initially");
        setupMockConfig();
        assertTrue(FarmApp.CONFIG != null, "Onboarding setup completed");
    ...
    public static void testPrinterMigrationAndBrimUpdate() {
        ...
        try {
            ...
            model.slice(configFile.getAbsolutePath(), new File(FarmApp.INSTANCE.getFilesDir(), "pla.gcode").getAbsolutePath(), null);
            String configPLA = readAllBytesFromFile(configFile);
            ...
            model.slice(configFile.getAbsolutePath(), new File(FarmApp.INSTANCE.getFilesDir(), "abs.gcode").getAbsolutePath(), null);
            String configABS = readAllBytesFromFile(configFile);
    ...
    public static void testHeadlessFullStateReset() {
        ...
        assertTrue(FarmApp.CONFIG == null, "Configuration must be cleared after environment reset");
    }
}
