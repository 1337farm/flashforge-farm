...
import com.flashforge.farm.slic3r.GLShaderProgram;
import com.flashforge.farm.slic3r.GLShadersManager;
import com.flashforge.farm.slic3r.Model;
import com.flashforge.farm.slic3r.PaintSession;
...
public class GLRenderer implements GLSurfaceView.Renderer {
    ...
    private String configValue(com.flashforge.farm.config.ConfigObject cfg, String key, String fallback) {
        String v = cfg != null ? cfg.get(key) : null;
        return v != null ? v : fallback;
    }

    private boolean boolConfig(com.flashforge.farm.config.ConfigObject cfg, String key, boolean fallback) {
    ...
    private boolean isPrimeTowerEnabled(com.flashforge.farm.config.ConfigObject cfg) {
    ...
    private double towerX(com.flashforge.farm.config.ConfigObject cfg) {
    ...
    private double towerY(com.flashforge.farm.config.ConfigObject cfg) {
    ...
    private double towerWidth(com.flashforge.farm.config.ConfigObject cfg) {
    ...
    private double towerDepth(com.flashforge.farm.config.ConfigObject cfg, double width) {
    ...
    public boolean tryStartPrimeTowerDrag(float screenX, float screenY) {
        ...
        com.flashforge.farm.config.ConfigObject cfg = FarmApp.buildCurrentConfigObject();
    ...
    public boolean finishPrimeTowerDrag(boolean commit) {
        ...
        if (commit) {
            ...
            FarmApp.LIVE_DIFF_PRINT.put("wipe_tower_x", xs);
            FarmApp.LIVE_DIFF_PRINT.put("wipe_tower_y", ys);
        }
        primeTowerPreviewX = Double.NaN;
        primeTowerPreviewY = Double.NaN;
        return true;
    ...
    public boolean setSelectedObject(int selectedObject) {
        ...
        Bus.SELECTED_OBJECT_CHANGED.postValue(new SelectedObjectChangedEvent());
        return true;
    }
    ...
    public boolean focusForDrag(int i) {
        ...
        if (selectedObjects.size() > 1 && selectedObjects.contains(i)) {
            ...
            selRotX = selRotY = selRotZ = 0;
            selScaleX = selScaleY = selScaleZ = 1;
            if (changed) Bus.SELECTED_OBJECT_CHANGED.postValue(new SelectedObjectChangedEvent());
            return true;
        }
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (lastConfigUid != FarmApp.CONFIG_UID) {
            configureBed();
        }
        if (bed.isValid()) {
        ...
        if (viewer == null && model != null) {
            ...
            for (int i = 0; i < model.getObjectsCount(); i++) {
                ...
                if (hovering && glModel.hoverProgress < 1) {
                    glModel.hoverProgress = Math.min(glModel.hoverProgress + dt / 150f, 1);
                    requestRender = true;
                } else if (!hovering && glModel.hoverProgress > 0) {
                    glModel.hoverProgress = Math.max(glModel.hoverProgress - dt / 150f, 0);
                    requestRender = true;
                }
                ...
                glModel.setColor(ColorUtils.blendARGB(baseColor, Color.WHITE, glModel.hoverProgress * 0.4f));
                glModel.render();
                ...
                if (!overlays.isEmpty()) {
                    ...
                    for (PaintOverlay o : overlays) {
                        o.model.setColor(ColorUtils.blendARGB(o.color, Color.WHITE, glModel.hoverProgress * 0.4f));
                        o.model.render();
                    }
        ...
        if (isInEmbossMode) {
            ...
            if (targetObj != -1 && targetObj < glModels.size()) {
                ...
                if (!raycastHits.isEmpty()) {
                    ...
                    Bus.EMBOSS_SURFACE_CLICKED.postValue(new EmbossSurfaceClickedEvent(position, normal));
                    return true;
                }
            }
            return false;
        ...
        if (isInMeasureMode) {
            ...
            if (targetObj != -1 && targetObj < glModels.size()) {
                ...
                if (!raycastHits.isEmpty()) {
                    ...
                    Bus.MEASURE_POINTS_CHANGED.postValue(new MeasurePointsChangedEvent(measurePointA, measurePointB));
                    return true;
                }
            }
            return false;
    ...
    private void configureBed() {
        try {
            ...
            bed.configure(FarmApp.getCurrentConfigFile());
        } catch (Exception e) {
