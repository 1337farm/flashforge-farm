# Gemini Handoff — flashforge-farm Mobile: Toolbar tools (2026-06-12)

Repo: `/home/cody/workspace/flashforge-farmMobileTestHandOffToGemini` (branch `master`, plain `git push` works)
...
```
APK filename embeds the LAST COMMIT hash — it does not change until you commit. Native lib = `libslic3r.so` (beam_native.cpp compiles into it). Crash logs: `adb logcat -b crash -d`. The slice config INI the app feeds the engine: `adb exec-out run-as com.codemastercody3d.flashforge-farmmobile cat files/slic3r_current.ini` — pulling this is how the last crash was solved; prefer it over guessing.
...
- Cheaper interim verification: pull the G-code (`adb exec-out run-as com.codemastercody3d.flashforge-farmmobile cat cache/temp.gcode > /tmp/t.gcode`) and check outer-wall loop start points cluster at the painted spot vs. scattered without paint. Or open the same G-code in desktop flashforge-farm with its Seams toggle.
