...
import com.flashforge.farm.components.SliceProgressBottomSheet;
import com.flashforge.farm.components.UnfoldMenu;
import com.flashforge.farm.components.bed_menu.BedMenu;
import com.flashforge.farm.components.bed_menu.CameraMenu;
import com.flashforge.farm.components.bed_menu.FileMenu;
import com.flashforge.farm.components.bed_menu.OrientationMenu;
import com.flashforge.farm.components.bed_menu.SliceMenu;
import com.flashforge.farm.components.bed_menu.TransformMenu;
...
import com.flashforge.farm.view.BedSwipeDownLayout;
import com.flashforge.farm.view.DividerView;
import com.flashforge.farm.view.GLView;
import com.flashforge.farm.view.SnackbarsLayout;
import com.flashforge.farm.view.ThemeBottomNavigationView;
import com.flashforge.farm.view.ThemeRailNavigationView;
...
public class BedFragment extends Fragment {
    ...
    private final Bus.Listener<NeedSnackbarEvent> onNeedSnackbar = e -> {
    ...
    private com.flashforge.farm.view.PaintModeView paintModeView;
    private com.flashforge.farm.view.MeasureModeView measureModeView;
    private com.flashforge.farm.view.VariableLayerHeightModeView variableLayerHeightModeView;
    private com.flashforge.farm.view.EmbossModeView embossModeView;
    ...
    public static File getTempGCodePath() {
        return tempExportingFile != null ? tempExportingFile : new File(FarmApp.INSTANCE.getCacheDir(), "temp.gcode");
    }
    ...
    public void enterPaintMode(int objectIndex) {
        enterPaintMode(objectIndex, com.flashforge.farm.render.GLRenderer.PAINT_MODE_COLOR);
    }
    ...
    public void enterMeasureMode() {
        ...
        measureModeView = new com.flashforge.farm.view.MeasureModeView(ctx, glView, this::exitMeasureMode);
    ...
    public void enterVariableLayerHeightMode() {
        ...
        variableLayerHeightModeView = new com.flashforge.farm.view.VariableLayerHeightModeView(ctx, glView, this::exitVariableLayerHeightMode);
    ...
    public void enterEmbossMode() {
        ...
        embossModeView = new com.flashforge.farm.view.EmbossModeView(ctx, glView, this::exitEmbossMode);
    ...
    public void onDestroy() {
        ...
        Bus.NEED_SNACKBAR.removeObserver(onNeedSnackbar);
        Bus.DISMISS_SNACKBAR.removeObserver(onDismissSnackbar);
    ...
    public void onResume() {
        super.onResume();
        glView.onResume();
        ConfigObject cfg = FarmApp.CONFIG.findPrinter(FarmApp.CONFIG.presets.get("printer"));
                ...
                if (glView.getRenderer().getModel() == null && !DEBUG_VIEWER) {
                    ...
                    new Thread(()->{
                        try {
                            ...
                            try {
                                FarmApp.ensureCompatibleSelection();
    ...
    private void selectMenu(Context ctx, boolean portrait, int slot) {
        if (glView.getRenderer().resetFlattenMode()) {
            glView.requestRender();
            Bus.FLATTEN_MODE_RESET.postValue(new FlattenModeResetEvent());
        }
    ...
    /** Desktop flashforge-farm plate grid column count (PartPlateList::compute_colum_count). */
    ...
    public void loadModel(File f, boolean preserveProjectLayout, int plateCount, ModelLoadCallback callback) throws Slic3rRuntimeError {
        ...
        if (plateCount > 1) {
            ...
            glView.queueEvent(() -> {
                ...
                for (com.flashforge.farm.slic3r.Model m : oldModels) {
                    if (m != null) m.release();
                }
            });
    ...
    public void splitSelectedObject() {
        ...
        glView.queueEvent(() -> {
            ...
            if (count <= 1) {
                Bus.NEED_SNACKBAR.postValue(new NeedSnackbarEvent(R.string.MenuToolbarSplitNothing));
                return;
            }
            ...
            ViewUtils.postOnMainThread(() -> {
                ...
                Bus.NEED_SNACKBAR.postValue(new NeedSnackbarEvent(R.string.MenuToolbarSplitDone, count));
            });
        });
    }
