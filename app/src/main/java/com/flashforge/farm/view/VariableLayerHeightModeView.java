    ...
    public VariableLayerHeightModeView(Context ctx, GLView glView, Runnable onExit) {
        ...
        clear.setOnClickListener(v -> {
            ...
            if (selectedObject != -1) {
                ...
                Bus.NEED_SNACKBAR.postValue(new NeedSnackbarEvent("Layer height profile cleared"));
                updateStatus();
            } else {
                Bus.NEED_SNACKBAR.postValue(new NeedSnackbarEvent("No object selected"));
            }
        });
        ...
        apply.setOnClickListener(v -> {
            ...
            if (selectedObject != -1) {
                try {
                    ...
                    Bus.NEED_SNACKBAR.postValue(new NeedSnackbarEvent("Adaptive layer height applied successfully"));
                    updateStatus();
                } catch (Exception e) {
                    Bus.NEED_SNACKBAR.postValue(new NeedSnackbarEvent("Failed to calculate adaptive layer height: " + e.getMessage()));
                }
            } else {
                Bus.NEED_SNACKBAR.postValue(new NeedSnackbarEvent("No object selected"));
            }
