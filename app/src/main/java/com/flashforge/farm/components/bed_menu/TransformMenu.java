...
public class TransformMenu extends ListBedMenu {
    ...
    private final Bus.Listener<SelectedObjectChangedEvent> onSelectionChanged = e -> updateSelectionDependentItems();
    ...
    protected List<SimpleRecyclerItem> onCreateItems(boolean portrait) {
        ...
        items.add(paintButton(R.string.MenuToolbarSupportPaint, com.flashforge.farm.render.GLRenderer.PAINT_MODE_SUPPORT));
        items.add(paintButton(R.string.MenuToolbarSeamPaint, com.flashforge.farm.render.GLRenderer.PAINT_MODE_SEAM));
        items.add(paintButton(R.string.MenuToolbarFuzzyPaint, com.flashforge.farm.render.GLRenderer.PAINT_MODE_FUZZY));
        items.add(paintButton(R.string.MenuToolbarColorPaint, com.flashforge.farm.render.GLRenderer.PAINT_MODE_COLOR));

        // --- Remove selected model (same as File menu "Remove model"; uses the thread-safe
        //     queueEvent wrapper so the GL renderer state isn't touched off-thread) ---
        items.add(selection(new BedMenuItem(R.string.MenuFileDelete, R.drawable.delete_outline_android_28)
                .onClick(v -> fragment.getGlView().deleteSelectedModel(() -> fragment.updateModel()))));
    ...
    private BedMenuItem notImplemented(int titleRes, int iconRes) {
        return new BedMenuItem(titleRes, iconRes).onClick(v ->
                Bus.NEED_SNACKBAR.postValue(new NeedSnackbarEvent(
                        FarmApp.INSTANCE.getString(R.string.MenuToolbarNotImplemented,
                                FarmApp.INSTANCE.getString(titleRes).replace("\n", " ")))));
    }
    ...
    public final class ScaleMenu extends UnfoldMenu {
        ...
        private final Bus.Listener<SelectedObjectChangedEvent> onSelectedObjectChanged =
                e -> {
                    stopScroll();
        ...
        private CharSequence formatTrackTitle(int res, double value, double mm) {
            SpannableStringBuilder sb = SpannableStringBuilder.valueOf(FarmApp.INSTANCE.getString(res, value, mm));
            sb.append(" d");
            int size = ViewUtils.dp(14);
            Drawable dr = ContextCompat.getDrawable(FarmApp.INSTANCE, R.drawable.edit_outline_28);
        ...
        protected void onCreate() {
            ...
            Bus.SELECTED_OBJECT_CHANGED.observeForever(onSelectedObjectChanged);
            setSelectionValues();
        }
        ...
        protected void onDestroy() {
            ...
            Bus.SELECTED_OBJECT_CHANGED.removeObserver(onSelectedObjectChanged);
            stopScroll();
        }
