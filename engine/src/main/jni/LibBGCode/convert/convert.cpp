...
BGCODE_CONVERT_EXPORT EResult from_ascii_to_binary(FILE& src_file, FILE& dst_file, const BinarizerConfig& config)
{
    ...
    static constexpr const std::string_view GeneratedBySlicer = "generated by FlashForge Farm"sv;
    ...
    if (!parser.parse([&](GCodeReader& r, const GCodeReader::GCodeLine& line) {
        ...
        size_t pos = sv_line.find(GeneratedBySlicer);
        if (pos != std::string_view::npos) {
            std::string_view version = trim(sv_line.substr(pos + GeneratedBySlicer.length()));
            pos = version.find(" ");
            if (pos != std::string_view::npos)
