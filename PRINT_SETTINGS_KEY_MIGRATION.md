# Handoff: Migrate Print-Settings option keys (PrusaSlicer → flashforge-farm)

## Context
This app (`flashforge-farmMobile`, branch `engine-swap`) swapped its native slicing engine from
PrusaSlicer-era `libslic3r` to the flashforge-farm/BBS engine. The Java UI and the bundled profiles
still use **PrusaSlicer option key names**, but the engine now uses **flashforge-farm names**. A previous
...
`app/src/main/java/com/flashforge/farm/fragment/PrintConfigFragment.java` builds its UI via
`OrcaPrintSettingsBuilder.build(...)`. That builder
(`app/src/main/java/com/flashforge/farm/fragment/OrcaPrintSettingsBuilder.java`) lists ~195 option
keys, but **114 of them use PrusaSlicer names that don't exist in the flashforge-farm engine's
...
- The UI reads/writes a setting's value by `def.key` (the def's flashforge-farm key) against the in-memory
...
- `ConfigObject` is built by parsing the profiles (old keys). To make UI key == stored key, the app
  ...
  (legacy → flashforge-farm), applied in `get/has/put/remove`. There is also a reverse map used by
  `Slic3rConfigWrapper` to keep profile categorization working.

So **every key you migrate must be changed in BOTH:**
1. `OrcaPrintSettingsBuilder.java` — rename the string literal to the flashforge-farm key.
...
The source of truth is the engine's `PrintConfigDef`. For each candidate flashforge-farm target key,
...
`Java_..._get_1print_1config_1def` (in `farm/farm_native.cpp`), a loop logging `nDef.keys()` to
...
Keys with **no flashforge-farm equivalent** → just remove them from the builder list (don't add to the
...
Format: `prusaslicer_key -> flashforge-farm_key`. Grouped. Confidence is "high" unless noted. Verify each.
...
2. In `OrcaPrintSettingsBuilder.java`, replace each old key string literal with its flashforge-farm key.
...
5. Build, install, verify:
   ...
   # APK: app/build/outputs/apk/debug/flashforge-farmMobile_*.apk
   adb install -r <apk>
   ```
...
- `Config.cpp` `ConfigBase::load()`: re-enabled `load_from_ini()` for non-JSON files (flashforge-farm had
  it commented out) — required for bed config + slicing to read FarmApp's flat .ini.
