...
{
    // boost::locale (and its ICU backend) is not vendored in this Android build, so Unicode NFC
    // normalization is unavailable. Input paths/names are passed through unchanged; this only
    // matters for the rare case of filenames containing decomposed combining characters.
    return src;
}
