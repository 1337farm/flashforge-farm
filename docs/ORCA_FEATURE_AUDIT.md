# flashforge-farm Mobile — Feature Parity Audit

Audit of what the flashforge-farm engine (now bundled via `app/src/main/jni/libslic3r`)
exposes vs. what the mobile UI actually surfaces. Goal: find features not yet surfaced in the mobile UI.
...
> The lists below are curated to the settings flashforge-farm **desktop actually shows in its tabs**.
...
Missing vs flashforge-farm desktop:
