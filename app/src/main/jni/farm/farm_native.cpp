...
GLShaderProgram* Slic3r::get_current_shader() {
    JNIEnv* env;
...
extern "C" {
    ...
    JNIEXPORT void JNICALL Java_com_flashforge_farm_slic3r_Native_get_1print_1config_1def(JNIEnv *env, jclass, jobject def) {
        jclass printConfigDefClass = env->FindClass("com/flashforge/farm/slic3r/PrintConfigDef");
        jmethodID printConfigAddOption = env->GetMethodID(printConfigDefClass, "addOption", "(Ljava/lang/String;Lcom/flashforge/farm/slic3r/ConfigOptionDef;)V");
        ...
        jclass configOptionDefClass = env->FindClass("com/flashforge/farm/slic3r/ConfigOptionDef");
        ...
        jfieldID typeField = env->GetFieldID(configOptionDefClass, "type", "Lcom/flashforge/farm/slic3r/ConfigOptionDef$ConfigOptionType;");
        jfieldID guiTypeField = env->GetFieldID(configOptionDefClass, "guiType", "Lcom/flashforge/farm/slic3r/ConfigOptionDef$GUIType;");
        ...
        jfieldID printerTechField = env->GetFieldID(configOptionDefClass, "printerTechnology", "Lcom/flashforge/farm/slic3r/ConfigOptionDef$PrinterTechnology;");
        ...
        jfieldID modeField = env->GetFieldID(configOptionDefClass, "mode", "Lcom/flashforge/farm/slic3r/ConfigOptionDef$ConfigOptionMode;");
        ...
        for (std::string key : nDef.keys()) {
            ...
            jobject typeValue = resolveEnum((char*) "com/flashforge/farm/slic3r/ConfigOptionDef$ConfigOptionType", (char*) typeStr);
            jobject guiTypeValue = resolveEnum((char*) "com/flashforge/farm/slic3r/ConfigOptionDef$GUIType", (char*) guiTypeStr);
            jobject labelValue = env->NewStringUTF(nCfgDef->label.c_str());
            jobject fullLabelValue = env->NewStringUTF(nCfgDef->full_label.c_str());
            jobject printerTechValue = resolveEnum((char*) "com/flashforge/farm/slic3r/ConfigOptionDef$PrinterTechnology", (char*) techStr);
            ...
            jobject modeValue = resolveEnum((char*) "com/flashforge/farm/slic3r/ConfigOptionDef$ConfigOptionMode", (char*) modeStr);
            ...
            if (isEnum) {
                // flashforge-farm stores enum labels/values as plain vectors on ConfigOptionDef
    ...
    JNIEXPORT jboolean JNICALL Java_com_flashforge_farm_slic3r_Native_model_1is_1big_1object(JNIEnv* env, jclass, jlong ptr, jint i) {
        ModelRef* model = (ModelRef*) (intptr_t) ptr;
        if (i < 0 || i >= (int) model->model.objects.size() || model->model.objects[i] == nullptr) return JNI_FALSE;
        ModelObject* obj = model->model.objects[i];
    ...
    JNIEXPORT jint JNICALL Java_com_flashforge_farm_slic3r_Native_model_1get_1extruder(JNIEnv* env, jclass, jlong ptr, jint i) {
        ModelRef* model = (ModelRef*) (intptr_t) ptr;
        if (i < 0 || i >= (int) model->model.objects.size() || model->model.objects[i] == nullptr) return -1;
        ModelObject* obj = model->model.objects[i];
        if (!obj->config.has("extruder")) return -1;
        const ConfigOptionInt* ext = dynamic_cast<const ConfigOptionInt*>(obj->config.option("extruder"));
        return ext ? ext->value : -1;
    }
    ...
    JNIEXPORT void JNICALL Java_com_flashforge_farm_slic3r_Native_model_1set_1extruder(JNIEnv* env, jclass, jlong ptr, jint i, jint extruder) {
        ModelRef* model = (ModelRef*) (intptr_t) ptr;
        if (i < 0 || i >= (int) model->model.objects.size() || model->model.objects[i] == nullptr) return;
        ModelObject* obj = model->model.objects[i];
        ...
        try {
            ...
            if (numFilaments > 1) {
                ...
                for (auto* mo : model->model.objects) {
                    __android_log_print(ANDROID_LOG_WARN, "FarmPaint", "object: %s, layer_config_ranges.size=%zu", mo->name.c_str(), mo->layer_config_ranges.size());
                    for (auto* v : mo->volumes) {
                        ...
                        __android_log_print(ANDROID_LOG_WARN, "FarmPaint", "volume: %s, maxPainted=%d", v->name.c_str(), volMax);
    ...
    JNIEXPORT jstring JNICALL Java_com_flashforge_farm_slic3r_Native_utils_1config_1eval(JNIEnv* env, jclass, jlong ptr, jstring cond) {
        ...
        try {
            ...
            env->ThrowNew(env->FindClass("com/flashforge/farm/slic3r/Slic3rRuntimeError"), err.what());
            return nullptr;
        }
    }
    ...
    JNIEXPORT void JNICALL Java_com_flashforge_farm_slic3r_Native_bed_1configure(JNIEnv* env, jclass, jlong ptr, jstring config_path) {
        ...
        try {
            ...
            // flashforge-farm renamed bed_shape -> printable_area and max_print_height -> printable_height.
    ...
    JNIEXPORT jboolean JNICALL Java_com_flashforge_farm_slic3r_Native_bed_1arrange(JNIEnv* env, jclass, jlong ptr, jlong model) {
        ...
        try {
            // flashforge-farm's arrange API takes the bed as Points (scaled bed shape) plus ArrangeParams,
    ...
    JNIEXPORT jboolean JNICALL Java_com_flashforge_farm_slic3r_Native_model_1has_1paint(JNIEnv* env, jclass, jlong modelPtr, jint objIdx) {
        ModelRef* m = (ModelRef*) (intptr_t) modelPtr;
        if (objIdx < 0 || objIdx >= (int) m->model.objects.size() || m->model.objects[objIdx] == nullptr) return false;
        ModelObject* obj = m->model.objects[objIdx];
        for (const Slic3r::ModelVolume* v : obj->volumes) {
