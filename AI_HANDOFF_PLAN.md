# AI Handoff: flashforge-farm Mobile Paint & Cut Tool Improvements
...
## Project Location
```
https://github.com/1337farm/flashforge-farm
```
...
- Transformation matrix computed in [GLRenderer.java](app/src/main/java/com/flashforge/farm/render/GLRenderer.java) matching the exact translation and rotation applied by the native cut engine.
- Slider listeners in [TransformMenu.java](app/src/main/java/com/flashforge/farm/components/bed_menu/TransformMenu.java) synchronize preview plane attributes in real time.
...
- UI SeekBars added to [PaintModeView.java](app/src/main/java/com/flashforge/farm/view/PaintModeView.java) to dynamically adjust height band size and angle limit thresholds.
...
Expose native connector methods in JNI. Add the following to [Native.java](app/src/main/java/com/flashforge/farm/slic3r/Native.java):
```java
...
Implement these methods in [farm_native.cpp](app/src/main/jni/farm/farm_native.cpp).
...
1. Add an "Add Connector" mode toggle button in `CutMenu` ([TransformMenu.java](app/src/main/java/com/flashforge/farm/components/bed_menu/TransformMenu.java)).
...
In [GLRenderer.java](app/src/main/java/com/flashforge/farm/render/GLRenderer.java), loop over the object's `cut_connectors` and render cylinder wireframes or solid shapes at their 3D positions on the cut plane so the user can see them before tapping "Apply Cut".
