...
import com.flashforge.farm.fragment.ProfileListFragment;
...
public class ConfigObject implements ProfileListFragment.ProfileListItem {
    ...
    // PrusaSlicer/FarmApp-era option keys -> flashforge-farm engine keys. Applied to every key entering
    // or leaving a ConfigObject so the UI (which looks up options by their flashforge-farm key), the stored
    ...
    // Reverse map (flashforge-farm -> legacy) so code that still categorizes by legacy key names can
    ...
    /** Resolve a config key to its flashforge-farm engine name (identity for keys already current/unknown). */
    public static String migrateKey(String key) {
        String mapped = KEY_MIGRATION.get(key);
        return mapped != null ? mapped : key;
    }

    /** Resolve an flashforge-farm key back to its legacy name (identity if it has none). */
    ...
    public boolean isSelected() {
        switch (profileListType) {
            case PROFILE_LIST_PRINT:
                return getTitle().equals(FarmApp.CONFIG.presets.get("print"));
            case PROFILE_LIST_FILAMENT:
                return getTitle().equals(FarmApp.CONFIG.presets.get("filament"));
            case PROFILE_LIST_PRINTER:
                return getTitle().equals(FarmApp.CONFIG.presets.get("printer"));
        }
        return false;
    }
    ...
    public String serialize() {
        ...
        sb.append("# generated by FlashForge Farm ").append(BuildConfig.VERSION_NAME).append("\n\n");
    ...
    public static ConfigObject createCustomPrinterProfile() {
        ConfigObject custom = new ConfigObject(FarmApp.INSTANCE.getString(R.string.IntroCustomProfileName));
    ...
    public static ConfigObject createCustomFilamentProfile() {
        ConfigObject genericFilament = new ConfigObject(FarmApp.INSTANCE.getString(R.string.IntroCustomProfileFilamentName));
