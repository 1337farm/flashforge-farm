// WipeTower2 for all non prusa printers, support all MMU device and toolchanger.
...
public:
    ...
    {
            // This class is only used by non PRUSA printers, so set the parameter appropriately.
            ...
            GCodeProcessor::s_IsPrusaPrinter = false;
                                                     ...
                                                     bool is_contact) const
{
    ...
    // Always initialize the tool_change_start_pos with a valid position
                                ...
                                unsigned int new_tool, float wipe_volume)
{
	...
	float length_to_extrude = volume_to_length(0.25f * std::accumulate(m_filpar[old_tool].ramming_speed.begin(), m_filpar[old_tool].ramming_speed.end(), 0.f),
										...
										layer_height_par);
    // Set ramming depth to 0 if ramming is disabled.
    ...
    // Keep wipe-depth planning consistent with toolchange_Wipe().
    // On the first layer, toolchange_Wipe() advances purge rows using
    // m_extra_flow * m_perimeter_width, while later layers use
    // m_extra_spacing_wipe * m_perimeter_width.
    // float dy = (is_first_layer() ? m_extra_flow : m_extra_spacing_wipe) * m_perimeter_width;
    // Use the same spacing here so reserved depth matches consumed depth
    // and first-layer purge segments do not leave visible gaps.
        ...
        const std::vector<WipeTowerInfo::ToolChange>& tool_changes) const
{
    ...
    // Allow calculation of the required depth and wipe volume for soluble toolchanges as well.
